...
class Application(tk.Tk):
    ...
    def __create_body(self):
        self.body_title = ttk.Label(self, text=self._description, font="Arial 20 bold")
        self.body_title.grid(row=0, column=1, columnspan=2)

        select_default_value: int | None = 0 if self._conversible_units_list else None
        entries_validation_cmd = (self.register(self._validate_entry), '%d %P')

        self.entry_1 = ttk.Entry(self, width=30, name='entry_1')
        self.entry_1.config(validate='key', validatecommand=entries_validation_cmd)
        self.entry_1.grid(row=1, column=1)
        self.select_1_value = tk.StringVar()
        self.select_1 = ttk.Combobox(self, textvariable=self.select_1_value, width=5)
        self.select_1['values'] = self._conversible_units_list
        self.select_1.grid(row=1, column=2)
        self.select_1.current(newindex=select_default_value)
        self.label_1 = ttk.Label(self, text='', font="Arial 9", foreground='red')
        self.label_1.grid(row=2, column=1)

        self.entry_2 = ttk.Entry(self, width=30, name='entry_2')
        ...
        self.select_2 = ttk.Combobox(self, textvariable=self.select_2_value, width=5)
        self.select_2['values'] = self._conversible_units_list
        self.select_2.grid(row=3, column=2)
        self.select_2.current(newindex=select_default_value)
        self.label_2 = ttk.Label(self, text='', font="Arial 9", foreground='red')
